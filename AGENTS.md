...
- `npm test` — run all vitest tests (301 tests, no Docker required)
...
- **Docker is required at startup.** The orchestrator calls `docker info` at boot and fatally exits if Docker is unreachable. In Cloud Agent environments, Docker must be started manually (`sudo dockerd &`) before running the app. After starting dockerd, fix socket permissions with `sudo chmod 666 /var/run/docker.sock` so the non-root user can access it.
